...
(() => {
	...
	PluginManager.registerCommand("Qianfan_APIMZ_zh", "chat", async (args) => {
		...
		(async () => {
			...
			try {
				// 调用百度千帆API生成事实性文本
				// const fetch = require('node-fetch'); // 用于发起 HTTP 请求的库

				// 函数用于调用文本生成API
				async function generateFactTextFromBaiduAPI() {
					try {
						...
						const response = await fetch(url, {
							...
							body: JSON.stringify({
								...
								"system": "请帮我总结一条一条过去的对话里面所有的事实性描述如果没有对话则不用总结" || "你是小花"
							}),
						});
						//console.log(customMemoryMessage);
						...
						if (response.ok) {
							...
							return data.result; // 假设返回的数据中有一个叫做 generatedText 的字段
						} else {
							// 在 response.ok 后添加以下代码：
							const responseData = await response.text();
							console.error('服务器返回的内容：', responseData);
				...
				// const inputText = '这是用于生成文本的输入数据';
				// 		const generatedText = await generateTextFromAPI(inputText);
				// 		console.log('生成的文本：', generatedText);				// 示例调用
				const baiduFactText = await generateFactTextFromBaiduAPI(); 
			// 使用适当的方法调用百度千帆API生成事实性文本
				var response
				var systemText
				const fs = require('fs');
				if (baiduFactText !== null && baiduFactText.trim() !== '') {
					// 如果生成的事实性文本不为空，则进行以下操作：
					// 将生成的事实性文本存储到本地txt文件中
					const filename = 'D:/Documents/RMMZ/Project3/js/plugins/abc.txt';
					fs.appendFile(filename, baiduFactText+"\n",'utf-8',(err)=>{}); // 存储文本到文件
					// let systemText = ''; // 声明并初始化 systemText 变量
					// 加载本地的事实性文本到系统中
					systemText = fs.readFile(filename, 'utf-8',(err)=>{}); // 从本地文件加载文本
					...
					response = await fetch(url, {
						...
						body: JSON.stringify({
							stream: true,
							messages: customMemoryMessage,
							system: String(args.system + systemText) || "你是小花" // 如果有加载到事实性文本则使用它，否则使用默认值
						}),
					});
				}
				else {
